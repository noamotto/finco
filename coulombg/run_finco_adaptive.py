...
def main():
    ...
    parser.add_argument('-k', type=int, default=0, action='store', help="Number of times to circle the last pole")
    ...
    adaptive_sampling(qs = (X+1j*Y).flatten(), S0 = S0,
                      ...
                      V = V, m = m, gamma_f = 1,
                      time_traj = CoulombGTimeTrajectory(n=args.n, k=args.k, t=halfcycle*2*args.t),
                      dt = 1e-4, drecord=1 / n_steps, n_jobs=n_jobs,
                      trajs_path=os.path.join(args.outdir, f'coulombg_{args.n}_{args.k}.hdf'))
