...
- The prefactor maps of all three "orders" with red highlight over the
nonphysical-diverging parts, put side by side.
- The prefactor maps of all three "orders" with red highlight over the
nonphysical-diverging parts and additional gree highlight showing the additional
manual filtering, put side by side.
...
pref4.tricontourf(np.real(trajs4.q0), np.imag(trajs4.q0), S_F4 * (np.imag(ts4) > 0),
                  cmap=stokes_cmap)
...
pref5.tricontourf(np.real(trajs5.q0), np.imag(trajs5.q0), S_F5 * (np.imag(ts5) > 0),
                  cmap=stokes_cmap)
...
pref6.tricontourf(np.real(trajs6.q0), np.imag(trajs6.q0), S_F6 * (np.imag(ts6) > 0),
                  cmap=stokes_cmap)
